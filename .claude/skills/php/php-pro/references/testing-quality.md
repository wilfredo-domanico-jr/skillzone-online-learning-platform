...
> For Laravel feature tests and Pest examples (factories, `actingAs`, `Http`/`Event`/`Queue`/`Notification`/`Storage` fakes, Sanctum auth testing), see the `laravel-specialist` skill's `references/testing.md` — this file focuses on the PHP-tooling side (static analysis, mocking, coverage) instead of duplicating those.
